...
func clearCooldownStateForAuth(auth *Auth, now time.Time) bool {
	...
	changed := false
	if auth.Quota.Reason != quotaAutoDisabledReason && (auth.Unavailable || !auth.NextRetryAfter.IsZero() || auth.Quota.Exceeded || !auth.Quota.NextRecoverAt.IsZero()) {
	...
	for _, state := range auth.ModelStates {
		...
		if state.Quota.Reason != quotaAutoDisabledReason && (state.Unavailable || !state.NextRetryAfter.IsZero() || state.Quota.Exceeded || !state.Quota.NextRecoverAt.IsZero()) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			...
			if result.Model != "" {
				...
				disableCooling := m.cooldownDisabledForAuth(auth)
				if recoverAt, okRecover := applyAuthFailureState(auth, result.Provider, result.Error, result.RetryAfter, now, disableCooling); okRecover {
...
func applyAuthFailureState(auth *Auth, provider string, resultErr *Error, retryAfter *time.Duration, now time.Time, disableCooling bool) (time.Time, bool) {
	if auth == nil {
		return time.Time{}, false
	}
