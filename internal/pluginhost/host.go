...
func (h *Host) recordCurrent(record capabilityRecord) bool {
	return h.identityCurrent(record.id, record.path, record.version)
}
...
// identityCurrent checks whether a plugin identity is still current,
// tolerating blank path and version for synthetic records used in tests.
func (h *Host) identityCurrent(id, path, version string) bool {
	...
	return h.pluginIdentityCurrent(id, path, version)
}
