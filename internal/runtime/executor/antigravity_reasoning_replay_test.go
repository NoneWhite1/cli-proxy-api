...
func TestPrepareAntigravityGeminiReasoningReplayPayloadAppendsStaleThoughtSignatureWithoutNullParts(t *testing.T) {
	...
	item := []byte(`{"type":"thought_signature","contentIndex":8,"partIndex":3,"thoughtSignature":"sig-text-long-enough"}`)
	...
	if got := parts[1].Get("thoughtSignature").String(); got != "sig-text-long-enough" {
		t.Fatalf("thoughtSignature = %q, want sig-text-long-enough; body=%s", got, out)
	}
}
