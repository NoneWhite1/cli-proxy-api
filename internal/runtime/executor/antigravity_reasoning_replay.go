...
func antigravityReplayPartWritePath(payload []byte, contentIndex int, partIndex int) string {
	...
	partsPath := fmt.Sprintf("request.contents.%d.parts", contentIndex)
	if parts := gjson.GetBytes(payload, partsPath); parts.IsArray() {
		return fmt.Sprintf("%s.%d", partsPath, len(parts.Array()))
	}
	return partsPath + ".0"
}
...
func insertAntigravityReasoningReplayItems(payload []byte, items [][]byte) ([]byte, bool) {
	...
	for _, item := range items {
		...
		case "thought_signature":
			...
			updated, ok := appendAntigravityThoughtSignaturePart(out, ci, sig)
			if !ok {
				continue
			}
			out = updated
...
func appendAntigravityThoughtSignaturePart(payload []byte, contentIndex int, thoughtSignature string) ([]byte, bool) {
	contents := gjson.GetBytes(payload, "request.contents")
	if !contents.IsArray() {
		return payload, false
	}
	arr := contents.Array()
	if contentIndex < 0 || contentIndex >= len(arr) {
		return payload, false
	}
	parts := arr[contentIndex].Get("parts")
	if !parts.IsArray() {
		return payload, false
	}
	partsArr := parts.Array()
	newParts := make([]any, 0, len(partsArr)+1)
	...
	newParts = append(newParts, map[string]any{"thoughtSignature": thoughtSignature})
	updated, err := sjson.SetBytes(payload, fmt.Sprintf("request.contents.%d.parts", contentIndex), newParts)
	if err != nil {
		return payload, false
	}
	return updated, true
