...
func ConvertOpenAIResponsesRequestToGemini(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	if contents.Exists() && contents.IsArray() {
		arr := contents.Array()
		if len(arr) > 0 && arr[len(arr)-1].Get("role").String() == "model" && !geminiResponseContentHasThought(arr[len(arr)-1]) {
...
func geminiResponseContentHasThought(content gjson.Result) bool {
