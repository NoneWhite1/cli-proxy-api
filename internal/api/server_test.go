...
func TestManagementPreheatPanelReadsBackendRefreshTimeState(t *testing.T) {
	...
	for _, token := range []string{
		...
		`preheat_needed`,
		`weekly_reset_at`,
		`weekly_gate_reset_at`,
		`function weeklyGateActive`,
		`weeklyGateActive(refresh)`,
...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	wantPriority := codexClientTestNonTemplatePriority(t, 1)
	if got := int(codexClientTestPriority(custom["priority"])); got != wantPriority {
...
func codexClientTestNonTemplatePriority(t *testing.T, rank int) int {
	t.Helper()
	...
	if err := json.Unmarshal(registry.GetCodexClientModelsJSON(), &payload); err != nil {
		t.Fatalf("failed to parse codex client models: %v", err)
	}
